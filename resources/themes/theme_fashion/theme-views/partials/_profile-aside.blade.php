    ...
    <ul class="nav nav-tabs nav--tabs-3 justify-content-start mb-0 d-none d-md-flex">
        ...
        <li class="nav-item">
            <a href="{{route('account-orders')}}"
               class="nav-link {{Request::is('account-orders*') || Request::is('account-order-details*') || Request::is('refund-details*') || Request::is('track-order/order-wise-result-view*') ? 'active' :''}}">{{ translate('my_order') }}
                ({{ auth('customer')->user()->orders->count()  }})</a>
        </li>
        <li class="nav-item">
            <a href="{{ route('user-restock-requests') }}"
               class="nav-link {{ Request::is('user-restock-requests*') ? 'active' :''}}">
...
<div class="offcanvas offcanvas-end text-capitalize" tabindex="-1" id="offcanvasProfile"
    ...
    <div class="offcanvas-body">
        <ul class="nav nav-tabs nav--tabs-3 p-2 flex-column">
            ...
            @php($restockProductCount = \App\Models\RestockProductCustomer::where('customer_id', auth('customer')->id())->count())
            <li class="nav-item">
                <a href="{{route('user-restock-requests')}}" class="nav-link {{Request::is('user-restock-requests') ? 'active' :''}}">
                    {{ translate('restock_request') }}
                    ({{ $restockProductCount  }})
                </a>
            </li>
            <li class="nav-item">
