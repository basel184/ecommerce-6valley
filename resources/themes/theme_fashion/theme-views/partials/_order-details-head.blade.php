<div class="d-flex flex-wrap align-items-center justify-content-between gap-3">
    ...
    <div>
        ...
        <div class="d-flex gap-3 align-items-center text-capitalize mb-2">
        ...
        @foreach ($order->offlinePayments->payment_info as $key=>$item)
            @if ($key != 'method_id' && $key != 'method_name')
                <div class="d-flex gap-3 align-items-center text-capitalize mb-2">
                    <span class="text-nowrap">{{translate($key)}} :</span>
                    <span class="badge font-regular badge-soft-success text-start text-wrap overflow-hidden">{{ $item }}</span>
                </div>
            @endif
        @endforeach
    @endif
        ...
        @if($order->order_type != 'POS')
            <li class="nav-item" role="presentation">
                <a href="{{ route('account-order-details-reviews', ['id'=>$order->id]) }}"
                   class="nav-link {{ Request::is('account-order-details-reviews')  ? 'active' : '' }}" >
                    {{ translate('reviews') }}
                </a>
            </li>
            ...
            <li class="nav-item" role="presentation">
                <a href="{{ route('track-order.order-wise-result-view', ['order_id' => $order['id']])}}"
                   class="nav-link {{Request::is('track-order/order-wise-result-view*')  ? 'active' :''}}" >
