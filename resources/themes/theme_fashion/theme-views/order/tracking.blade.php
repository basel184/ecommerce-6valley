                    ...
                    <tr>
                        <td class="border-0" data-label="{{translate('order_ID')}}">
                            @if(auth('customer')->check())
                                <span class="cursor-pointer custom-text-link thisIsALinkElement"
                                      data-linkpath="{{ route('account-order-details', ['id'=>$order->id]) }}">
                                    #{{$order->id}}
                                </span>
                            @else
                        ...
                        <td class="border-0" data-label="{{translate('vendor')}}">
                            @if($order->seller_is == 'seller')
                                <a href="{{ route('shopView', ['id'=> $order->seller->shop['id']]) }}"
                                   class="text-title custom-text-link">
                        ...
                        <td class="border-0" data-label="{{translate('item')}}">
                            <div class="ms-3">{{count($order->details)}}</div>
                        </td>
                    </tr>
                    </tbody>
                </table>
                ...
                <h3 class="mb-3 mb-lg-4 d-none d-sm-block">{{ translate('tracking_status') }}</h3>
                        ...
                        <div class="tracking-flow-wrapper pt-lg-3">
                            <div class="tracking-flow-item active">
                                ...
                                <div>
                                    ...
                                    <small class="d-block">{{date('d M, Y h:i A',strtotime($order->created_at))}}</small>
                                </div>
                            </div>

                            @if ($order['order_status']!='returned' && $order['order_status']!='failed' && $order['order_status']!='canceled')
                                <div class="tracking-flow-item {{($order['order_status']=='processing') || ($order['order_status']=='processed') || ($order['order_status']=='out_for_delivery') || ($order['order_status']=='delivered')?'active' : ''}}">
                                    <div class="img">
                                    ...
                                    <div>
                                        ...
                                        @if(($order['order_status']=='processing') || ($order['order_status']=='processed') || ($order['order_status']=='out_for_delivery') || ($order['order_status']=='delivered'))
                                            <small class="d-block">
                                                @if(\App\Utils\order_status_history($order['id'],'processing'))
                                                    {{date('d M, Y h:i A',strtotime(\App\Utils\order_status_history($order['id'],'processing')))}}
                                                @endif
                                            </small>
                                        @endif
                                    </div>
                                </div>
                                <div class="tracking-flow-item {{($order['order_status']=='out_for_delivery') || ($order['order_status']=='delivered')? 'active' : ''}}">
                                    <div class="img">
                                    ...
                                    <div>
                                        ...
                                        @if(($order['order_status']=='out_for_delivery') || ($order['order_status']=='delivered'))
                                            <small class="d-block">
                                                @if(\App\Utils\order_status_history($order['id'],'processing'))
                                                    {{date('d M, Y h:i A',strtotime(\App\Utils\order_status_history($order['id'],'out_for_delivery')))}}
                                                @endif
                                            </small>
                                        @endif
                                    </div>
                                </div>
                                <div class="tracking-flow-item {{($order['order_status']=='delivered')?'active' : ''}}">
                                    <div class="img">
                                    ...
                                    <div>
                                        ...
                                        @if($order['order_status']=='delivered')
                                            <small class="d-block">
                                                @if(\App\Utils\order_status_history($order['id'],'processing'))
                                                    {{date('d M, Y h:i A',strtotime(\App\Utils\order_status_history($order['id'],'delivered')))}}
                                                @endif
                                            </small>
                                        @endif
                                    </div>
                            ...
                            @elseif(in_array($order['order_status'], ['returned', 'canceled']))
                                <div class="tracking-flow-item active">
                                    <div class="img">
                                        <img loading="lazy" src="{{theme_asset('assets/img/track/'.$order['order_status'].'.png')}}" alt="">
                                    </div>
                                    <span class="icon"><i class="bi bi-check"></i></span>
                    ...
                    @if ($order['order_status']!='returned' && $order['order_status']!='failed' && $order['order_status']!='canceled')
                        ...
                        <ul>
                            ...
                            @if ($order_verification_status && $order->order_type == "default_type")
                                <li>
                            ...
                            <li>
                                ...
                                @if($order['order_status']=='confirmed' || $order['order_status']=='processing' || $order['order_status']=='delivered')
                                    <strong>
                ...
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header px-sm-5 border-0 mx-1">
                            <div>
                                <h1 class="modal-title fs-5">{{ translate('order')}} # {{$order->id}}</h1>

                                @if ($order_verification_status && $order->order_type == "default_type")
                                    <h5 class="small">{{translate('verification_code')}}
                                        : {{ $order->verification_code }}</h5>
                                @endif
                            </div>
                            ...
                            <div class="product-table-wrap">
                                <div class="table-responsive">
                                    <table class="table text-capitalize text-start align-middle">
                                        ...
                                        @foreach($orderDetails as $key=>$order_details)
                                            ...
                                            <tr>
                                                <td>
                                                    <div class="media align-items-center gap-3">
                                                        ...
                                                        <div>
                                                            ...
                                                            @if($order_details->payment_status == 'paid' && $productDetails->digital_product_type == 'ready_product')
                                                            ...
                                                            @elseif($order_details->payment_status == 'paid' && $productDetails->digital_product_type == 'ready_after_sell')
                                                ...
                                                <td class="text-end">
                                                    {{finalSaudiPrice($order_details['price']*$order_details['qty'])}}
                                                </td>
                                            </tr>
                            ...
                            <div class="bg-light rounded border p3">
                                <div class="table-responsive">
                                    <table class="table __table text-end table-align-middle text-capitalize">
                                        ...
                                        <tr>
                                            <td class="text-dark">
                                                {{finalSaudiPrice($sub_total)}}
                                            </td>

                                            @if ($order->order_type == 'default_type')
                                                <td class="text-dark">
                                                    {{finalSaudiPrice($order->is_shipping_free ? $total_shipping_cost-$order->extra_discount:$total_shipping_cost)}}
                                                </td>
                                            @endif

                                            <td class="text-dark">
                                                {{finalSaudiPrice($total_tax)}}
                                            </td>
                                            <td class="text-dark">
                                                -{{finalSaudiPrice($total_discount_on_product)}}
                                            </td>
                                            <td class="text-dark">
                                                - {{finalSaudiPrice($coupon_discount)}}
                                            </td>
                                            @if ($order->order_type == 'POS')
                                                <td class="text-dark">
                                                    - {{finalSaudiPrice($extra_discount)}}
                                                </td>
                                            @endif
                                            <td class="text-dark">
                                                {{finalSaudiPrice($sub_total+$total_tax+$total_shipping_cost-($order->discount)-$total_discount_on_product - $coupon_discount - $extra_discount)}}
                                            </td>
                                        </tr>
                                        </tbody>
                                    </table>
