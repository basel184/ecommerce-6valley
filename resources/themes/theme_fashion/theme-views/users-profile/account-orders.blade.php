...
@section('title', translate('my_order').' | '.$web_config['company_name'].' '.translate('ecommerce'))
                ...
                <div class="mb-4">
                    <ul class="nav nav-tabs nav--tabs-2" role="tablist">
                        <li class="nav-item" role="presentation">
                            <a href="{{route('account-orders', ['show_order'=>'ongoing'])}}" class="nav-link {{ (request('show_order')=='ongoing' || request('show_order') == null) ? 'active' : '' }}">{{translate('ongoing')}}</a>
                        </li>
                        <li class="nav-item" role="presentation">
                            <a href="{{route('account-orders', ['show_order'=>'previous'])}}" class="nav-link {{ request('show_order')=='previous' ? 'active' : '' }}">{{translate('previous')}}</a>
                        </li>
                    </ul>
                        ...
                        <div class="search-form-2 search-form-mobile">
                            ...
                            <button type="submit" class="clear border-0">
                                @if (request('search') != null)
                                    <a href="{{route('account-orders',['show_order'=>request('show_order')])}}">{{translate('clear')}}</a>
                                @else
                                    ...
                                    <tr class="bg-section">
                                            ...
                                            <div class="cart-product ">
                                                ...
                                                <div class="cont">
                                                    <div  class="name text-title text-capitalize cursor-pointer thisIsALinkElement" data-linkpath="{{ route('account-order-details', ['id'=>$order->id]) }}">
                                                        ...
                                                        @elseif($order->seller_is == 'admin')
                                                        <div class="text-base mb-1">{{ $web_config['company_name'] }}</div>
                                                        @endif
                                        ...
                                        <td class="text-center">
                                            <div class="d-flex justify-content-center column-gap-2">
                                                <span class="text-base price-red ">
                                    ...
                                    <div class="cont">
                                        <div class="name text-title text-capitalize thisIsALinkElement" data-linkpath="{{ route('account-order-details', ['id'=>$order->id]) }}">{{translate('order')}}# {{$order->id}}</div>
                                            @if($order->seller_is == 'seller')
                                                <div class="text-base mb-1">{{ isset($order->seller->shop) ? $order->seller->shop->name : translate('shop_not_found') }}</div>
                                            @elseif($order->seller_is == 'admin')
                                            <div class="text-basa mb-1">{{ $web_config['company_name'] }}</div>
                                            @endif
                                        ...
                                        <div class="text-basa mb-1">
                                            <span>{{translate('total_price')}} :
                                                <strong>{{webCurrencyConverter($order['order_amount'])}}</strong>
                                            </span>
                                        </div>
                                        <div class="text-basa mb-1">
                                            <span>{{translate('qty')}} :
