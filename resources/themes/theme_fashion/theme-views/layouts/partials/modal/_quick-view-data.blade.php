            ...
            <form class="cart add-to-cart-details-form addToCartDynamicForm" action="{{ route('cart.add') }}"
                ...
                <div class="price">
                    <h4 class="d-flex align-items-end gap-2">
                        ...
                        <span class="discounted-badge-element {{ $product->discount > 0 ? '' : 'd--none' }}">
                            @if ($product->discount > 0 && $product->discount_type === "percent")
                                <span class="badge bg-base discounted_badge">{!!webCurrencyConverterWithImage($product->discount)!!}%</span>
                            @else
                                <span class="badge bg-base discounted_badge">{{translate('save')}} {!!webCurrencyConverterWithImage($product->discount)!!}</span>
                            @endif
                        </span>
                    </h4>
                </div>
                ...
                <div class="btn-grp">
                    ...
                    <div class="product-add-and-buy-section d--flex flex-wrap gap-2" {!! $firstVariationQuantity <= 0 ? 'style="display: none;"' : '' !!}>
                        ...
                        @else
                            <a href="javascript:"
                               class="btn btn-base text-capitalize product-add-to-cart-button"
                               type="button"
...
<span id="quick-view-product-id" data-product-id="{{ $product['id'] }}"></span>
